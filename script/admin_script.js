var questions = []//this should be empty by default as all the questions will be added to this object from firebase's realtime database
questions_temp =
    {
        key: "",
        question: "",
        answer: "",
        options: []//array is used to store the 4 choices (mcq)
    }

var quizHeader = document.getElementById("quizHeader")
var quizBody = document.getElementById("quizBody")
var qNum = 0
var answers = []//array to show the options
var minutes = 0
var seconds = 0
...
var rVal = ''
var resultdb = firebase.firestore(); //initialising firestore db to store the results of the quiz
...
function startQuiz(){
    ...
    appendQuestion() //function is called to display the questions set by the teacher along with the other elements of the quiz panel
    interval = setInterval(function(){
        ...
        if(formattedMinutes == 02 && formattedSeconds == 15){
            alert("time up!")
            appendResult()
        }
        document.getElementById("timer").innerHTML = `${formattedMinutes}:${formattedSeconds}`
    }, 1000)//a regular timer function, counts upto 2 mins 15 seconds and then closes the quiz
    
}

//this function displays the quiz panel with the header, timer, questions, answers and the next button
...
//an option must be selected to move to the next question. This function checks this condition
...
//once the finish button is clicked or the time runs out, the appendResult function is called which prompts for the registration number and 
//then displays the results in a table
function appendResult(){

    var person = prompt("Enter Reg_Num", );
    ...
    divBody += `<td>${person}</td>`
    divBody += "</tbody></table>"
    quizBody.innerHTML = divBody

  
    //In firestore db data is stored in the form of documents. For our project the document id is auto generated
    // The following code snippet adds a new document in collection named Results
    resultdb.collection("Results").add({
    Reg_Num: person,
    Percentage:(correctQuestions/questions.length)*100,
    Marks: correctQuestions
    },{merge:true})
    .then(() => {
...
function getQuestions(){//Gets all the questions from firebase and save them in questions object created in the first line of this file
...
function appendAllQuestions(){
    for(var j=0; j<questions.length; j++){
        ...
        var divBody = "<li style='background-color: white; border-style: solid; border-radius: 30px; padding: 10px 30px; margin-bottom: 10px'>"
...
function addQuestion(){
    if(!addingRec){//check if a record is'nt already being added
        ...
        var htmlDesign = '<li class="panelLi" style="background-color:white; border-radius: 30px; padding: 10px 30px">'
...
function addRec(id){
    ...
    // we have to assign a unique key to our questions object first and then push it to the firebase realtime db
...
//if an added question is being edit the following function is called. this ensures that the question is overwritten instead of adding it as a new one, even in firebase using the key
function editRec(id){
    if(!addingRec){
        addingRec = true
        editingRec = true
        let key = id.id
        console.log(key)
        let numOfOtions = id.parentNode.parentNode.childNodes.length - 1//checks number of li's inside optionsUl and -1(as there one li for buttons)
        let optionVals = []
        let answerVal = ''
        ...
        if(id.parentNode.parentNode.childNodes[i].className.indexOf('active') !== -1)//if the class of option is 'active', that means that it's the correct answer
            answerVal = optionVals[i]//assign this option value to the answer object
        }
        // stores the whole li which has our quesion,answer and options to the 'rVal', so if we do'nt update the edited question, we may restore the recent value
        rVal = id.parentNode.parentNode.parentNode.parentNode.childNodes[editQNum]

        //every object has to be put inside a input element, this will be almost same as we did in addQuestion()
...
function discardRec(id){
    addingRec = false
    id.parentNode.parentNode.parentNode.remove() //deletes a record
}